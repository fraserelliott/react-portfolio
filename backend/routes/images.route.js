...
async function retrieveFromDB(req, res, next) {
  try {
    ...
    if (isNaN(id))
      return res.status(400).json({ error: "Invalid id or not a number" });
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      await req.dbImage.update(
        { url: req.image.url, publicId: req.image.public_id },
        { transaction: tx },
      );
      await tx.commit();
  ...
  async (req, res) => {
    try {
      if (!req.body.reference)
        return res
          .status(400)
          .json({ error: "No reference provided to update" });
