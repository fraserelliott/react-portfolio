...
router.post('/', auth.validateToken, inputValidation.validate(postSchema), async (req, res) => {
  try {
    const { title, content, featured, summary, repoLink, tags } = req.body;

    // Verify all provided tags exist
    const existingTags = await verifyTags(tags);
    if (!existingTags) return res.status(400).json({ error: 'One or more tag IDs are invalid.' });

    // Create post and associate tags
    const post = await Post.create({ title, content, featured, summary, repoLink });
...
// TODO: separate route to not include content for faster fetch where content isn't needed for home or projects pages
...
router.put('/:id', auth.validateToken, inputValidation.validate(postSchema), async (req, res) => {
  try {
    ...
    await post.update({ title, content, featured, summary, repoLink });
